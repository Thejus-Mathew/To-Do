...
function Search() {
  ...
  const handleSearch = (description) =>{
    ...
    if (description.search(search.toLowerCase()) != -1) {
      return true
    }else{
      return false
    }
  ...
  return (
    ...
    <MDBTable align='middle'>
      ...
      <MDBTableBody>
        {
          todos.filter(item=>handleSearch(item?.description)).length>0?todos.filter(item=>handleSearch(item?.description)).filter(item=>handleFilter(item)).map((item,index)=>(
          ...
          <tr>
            <td colSpan={5}>Nothing found for the task named "{search}"</td>
          </tr>
        }
      </MDBTableBody>
    </MDBTable>
